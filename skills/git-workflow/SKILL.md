...
when_to_use: >-
  ...
  Also invoked by scenario-standards during Implementation and DevOps phases.
  Trigger phrases: /git-workflow, git 工作流, commit, 提交, push, 推送, pull, 拉取, branch, 分支, 暂存, staged, 拆分, split
...
- `/git-workflow commit`: Smart commit with auto-splitting
- `/git-workflow push`: Safe push with auto-sync
- `/git-workflow pull`: Safe pull with rebase
- `/git-workflow branch <name>`: Create and switch to branch (safe mode)
- `/git-workflow status`: Show current Git status with suggestions
...
```
User: /git-workflow branch feat/user-profile
...
Resolve conflicts manually, then run `/git-workflow continue` to continue rebase.
```
...
AI:  Syncing with remote...
     ...
     You can now start developing. Use `/git-workflow commit` to commit changes.
```
