...
This document defines the Git workflow standards for projects using the `git-workflow` skill.
...
4. **Body**:
   ...
   - Max 3 lines
5. **AI Author Tag**: Required for commits generated using `git-workflow` skill
...
Commits generated using `git-workflow` skill must add:
...
| Merge conflicts         | Use `/git-workflow pull` to auto-rebase, resolve conflicts manually           |
...
- Use `/git-workflow status` to check current Git status
